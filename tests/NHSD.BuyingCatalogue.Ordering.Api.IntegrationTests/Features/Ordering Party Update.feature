...
Background:
    ...
    Given Orders exist
        | Description    | OrganisationId                       | OrganisationName | OrganisationOdsCode | OrganisationAddressPostcode | OrganisationContactEmail | OrderStatusId | Created    | LastUpdated | LastUpdatedBy                        |
        | Description 01 | 4af62b99-638c-4247-875e-965239cd0c48 | Hampshire CC     | 432432              | LS 1 3AP                    | Fred.robinson@email.com  | 1             | 11/05/2020 | 11/05/2020  | 335392e4-4bb1-413b-9de5-36a85c9c0422 |
    And the user is logged in with the Buyer role for organisation 4af62b99-638c-4247-875e-965239cd0c48

@4616
Scenario: 1. Update an ordering party results in persisted data
    Given an order party update request exist for order with description Description 01
    And the update request for order with description Description 01 has a contact
        | FirstName     | LastName     | EmailAddress        | TelephoneNumber |
        | TestFirstName | TestLastName | TestEmail@email.com | TestNumber      |
    And the order party update request for order with description Description 01 has a address
        | Line1 | Line2    | Line3     | Line4        | Line5           | Town         | County        | Postcode | Country        |
        | 4     | TestRoad | Test Lane | Test Village | Testing Village | Testing Town | TestingCounty | TE 1 ST  | United Kingdom |
    And the order party update request for order with description Description 01 has a Name of TestCare Center
    And the order party update request for order with description Description 01 has a OdsCode of ODS1
    When the user makes a request to update the order party on the order with the Description Description 01
    Then a response with status code 204 is returned
    And the order with description Description 01 is updated and has a primary contact with data
        | FirstName     | LastName     | Email               | Phone      |
        | TestFirstName | TestLastName | TestEmail@email.com | TestNumber |
    And the order with description Description 01 is updated and has a Organisation Address with data
        | Line1 | Line2    | Line3     | Line4        | Line5           | Town         | County        | Postcode | Country        |
        | 4     | TestRoad | Test Lane | Test Village | Testing Village | Testing Town | TestingCounty | TE 1 ST  | United Kingdom |
...
Scenario: 4. If a user is not authorised then they cannot update the ordering-party
    ...
    And an order party update request exist for order with description Description 01
    And the order party update request for order with description Description 01 has a Name of TestCare Center
    When the user makes a request to update the order party on the order with the Description Description 01
